...
int sas_from_string_to_int(string operand) { return stoi(operand); }
...
void sas_read_asm_string(string s, int &string_num, string &command,
                         string &operand) {
  command = "";
  operand = "";
  while (true) {
...
vector<string> commands_string_id = {"READ", "WRITE", "ADD",   "SUB",  "DIV",
...
void sas_manager(string path_to_file, string path_to_obj_file) {
  try {
    ...
    while (getline(file1, s) && ++counter) {
      ...
      if (find(commands_string_id.begin(), commands_string_id.end(), command) !=
          commands_string_id.end()) {
